...
class StavropolParser(BaseParser):
    ...
    async def get_new_news(self, last_news_date=None, max_news=3) -> [Post]:
        ...
        articles_block = soup.find_all('td', class_='news_render_lenta_header', limit=max_news)
        urls = []

        for article in articles_block:
            try:
                link = 'https:' + article.find_next('a').get('href')
                urls.append(link)
...
if __name__ == '__main__':
    asyncio.run(StavropolParser().get_new_news())
