...
@dataclass
class VoronezhParser(BaseParser, BaseRequest):
    city: SiteModel = SiteModel.VORONEZH
    name: str = 'voronezh'
    __base_url: str = 'https://riavrn.ru'
    __news_url: str = 'https://riavrn.ru/news/?from=main_sidenews'
    referer: str = 'https://riavrn.ru/news/?from=main_sidenews'
    ...
    async def find_news_urls(self) -> list[str]:
        ...
        main_ul = soup.find('ul', class_='news-list no-bullet')
        items = main_ul.find_all(
            'li',
            limit=5,
        )
        if not items:
    ...
    def find_body(self, soup: BeautifulSoup) -> str | None:
        ...
        content = soup.find(
            'div',
            class_=lambda v: find_value(v, 'article-text'),
        )
        if not content:
            return None
        ...
        for p in ps:
            p = p.text
            if 'читайте в материале' in p:
                continue
            if 'Подробнее – в материале' in p:
                return body
    ...
    def find_photos(self, soup: BeautifulSoup) -> list:
        photos = []
        content_div = soup.find('div', class_='pic')
        if content_div:
        ...
        photo_divs = content.find_all('div', class_='photo otr-left clearfix')
        if not photo_divs:
            return photos
        for photo_raw in photo_divs:
            img = photo_raw.find('img')
            if img:
                img = img.get('src')
