...
@dataclass
class RioParser(BaseParser, BaseRequest):
    request_object: BaseRequest
    city: SiteModel = SiteModel.RIO
    name: str = 'rio'
    __base_url: str = 'https://www.cnnbrasil.com.br'
    __news_url: str = 'https://www.cnnbrasil.com.br/tudo-sobre/rio-de-janeiro-estado/'
    referer: str = 'https://www.cnnbrasil.com.br/tudo-sobre/rio-de-janeiro-estado/'
    ...
    async def find_news_urls(self) -> list[str]:
        ...
        items = soup.find_all('li', class_='home__list__item', limit=3)
        if not items:
    ...
    def find_photos(self, soup: BeautifulSoup) -> list:
        ...
        main_div = soup.find(
            'picture',
            class_='img__destaque',
        )
        if main_div:
            photo = main_div.find('img').get('src')
            photos.append(photo)
        return photos
...
async def test() -> None:
    ...
    parser = RioParser(request_object=request_obj)
