...
@dataclass
class ShimkentParser(BaseParser, BaseRequest):
    city: SiteModel = SiteModel.SHIMKENT
    name: str = 'shimkent'
    __base_url = 'https://otyrar.kz'
    __news_url = 'https://otyrar.kz/?amp'
    referer = 'https://otyrar.kz/?amp'
    ...
    async def find_news_urls(self) -> list[str]:
        ...
        news = soup.find_all('div', class_='td_module_mob_1 td_module_wrap td-animation-stack')
        if not news:
    ...
    def find_title(self, soup: BeautifulSoup) -> str | None:
        main_block = soup.find('div', class_='td-post-header')
        title = main_block.find('h1').text.strip()
        return title

    def find_body(self, soup: BeautifulSoup) -> str | None:
        content = ''
        main_block = soup.find('div', class_='td-post-content')
    ...
    def find_photos(self, soup: BeautifulSoup) -> list[str] | list:
        image_urls = []
        main_block = soup.find('div', class_='td-post-content')
        all_images = main_block.find_all('amp-img')
