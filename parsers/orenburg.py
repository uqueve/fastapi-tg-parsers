...
@dataclass
class OrenburgParser(BaseParser, BaseRequest):
    city: SiteModel = SiteModel.ORENBURG
    name: str = 'orenburg'
    __base_url = 'https://orenday.ru/'
    __news_url = 'https://orenday.ru/'
    ...
    async def find_news_urls(self) -> list[str]:
        ...
        main_block = soup.find('div', class_='last-news')
        news = main_block.find_all('a', class_=lambda v: find_value(v, 'new-title'))
        if not news:
    ...
    def find_photos(self, soup: BeautifulSoup) -> list[str] | list:
        image_urls = []
        main_photo = soup.find('div', class_='foto-news')
        if main_photo:
            image_raw = main_photo.find('img')
            if image_raw:
                img = image_raw.get('src')
        ...
        for img in all_images:
            ...
            if not image_raw.startswith('http:'):
                image_raw = self.__base_url + image_raw
