...
class OpenAIService:
    ...
    def make_request(self, texts: str, prompt_for: str = "body", language: str = "русском"):
        if prompt_for == "body":
            prompt = (
                f'Перепиши текст более художественно на {language} языке, сохраняя смысл, и сократи его до размера '
        ...
        elif prompt_for == "title":
            prompt = (
                f"Перепиши заголовок новости более художественно на {language}, сохраняя смысл. "
                f"Заголовок не может быть длиннее 150 символов, строго! Убери лишние упоминания источника в заголовке. "
                f"Не забудь писать исключительно на {language} языке. Заголовок для обработки: {texts}"
            )
        try:
